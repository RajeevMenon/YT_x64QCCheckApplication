...
#End Region
    ...
    Public Function ProcessStepNo120_05_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            AddWriteField("120_01_00_01", MainForm.Initial, WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
                WMsg.ShowDialog()
    ...
    Public Function ProcessStepNo160_01_00_01(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            ProcessStepReturn.StepNo = "160_01_00_01"
            ProcessStepReturn.ActivityToCheck = "Terminal cover wiring diagram label"
            ProcessStepReturn.SinglePointAction.SPI_Message = "Check the presence of wiring diagram label"
            ProcessStepReturn.SinglePointAction.ImagePath_SPI_1 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\Common\" & "F9220BH_Yes.jpg"
            ProcessStepReturn.SinglePointAction.ImagePath_SPI_2 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\Common\" & "F9220BH_No.jpg"
            ProcessStepReturn.SinglePointAction.ImagePath_SPI_Correct = "F9220BH_Yes.jpg"

            Dim WriteFields As New List(Of OpenPdfOperation_x64.WriteField)
            Dim ResultValue As String = "F9220BH_Yes"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
    ...
    Public Function ProcessStepNo170_03_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            ProcessStepReturn.MakeUserInputAction.UserInputSaveConnectionString = "QR_CHECK"
    ...
    Public Function ProcessStepNo170_04_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            If CustOrd.EU_COUNTRY = "SA" And Link.PlantID = "5Q00" Then
                ...
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_1 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "Saudi_Made.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_2 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "No_Stickers.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_Correct = "Saudi_Made.jpg"
                ResultValue = $"[Saudi Made] ✓" '$"SAUDI MADE ✓"
            ElseIf CustOrd.EU_COUNTRY <> "SA" And Link.PlantID = "5Q00" Then
                ...
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_1 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "Saudi_Made.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_2 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "No_Stickers.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_Correct = "No_Stickers.jpg"
                ResultValue = $"[None] ✗" '$"SAUDI MADE ✗"
            ElseIf Link.PlantID = "6Z00" Then
                ...
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_1 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "QR_Sticker.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_2 = MainForm.Setting.Var_52_SinglePntInst_ImagePath & "YTA\170\" & "No_Stickers.jpg"
                ProcessStepReturn.SinglePointAction.ImagePath_SPI_Correct = "No_Stickers.jpg"
                ResultValue = $"[None] ✗"
            End If
    ...
    Public Function ProcessStepNo180_01_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            ' Base relative paths for Document API
            Dim QicFolderBase As String = "Production Complete Documents\QICDOC\"
            Dim prodNo As String = If(CustOrd.PROD_NO, String.Empty)
            Dim lotNo As String = If(CustOrd.SHIP_LOT, String.Empty)
            ...
            ' Create required remote directories using DirMgr Wrapper
            Dim prodDirPath As String = QicFolderBase & prodNo
            If Not DirMgr.Exists(prodDirPath) Then
                DirMgr.CreateDirectory(prodDirPath)
            End If

            Dim lotDirPath As String = prodDirPath & "\" & lotNo
            If Not DirMgr.Exists(lotDirPath) Then
                DirMgr.CreateDirectory(lotDirPath)
            End If

            Dim relYtaQicFolder As String = "Production Complete Documents\YTA_QIC\"
            Dim sourcePdfRel As String = relYtaQicFolder & CustOrd.SERIAL_NO & ".pdf"
            Dim sourceXlsRel As String = relYtaQicFolder & CustOrd.SERIAL_NO & ".xls"

            Dim targetFileName As String = "LINE-" & CustOrd.LINE_NO & "-" & CustOrd.INDEX_NO & "(1)-CQIC"
            Dim targetPdfRel As String = lotDirPath & "\" & targetFileName & ".pdf"

            ' Use FileMgr to check if the PDF was already generated by Headquarters/previous step
            If FileMgr.Exists(sourcePdfRel) And CustOrd.MTC = "GO" Then
                If Not FileMgr.Exists(targetPdfRel) Then
                    ' Download to local temp, rename it, upload to Target folder, then cleanup
                    Dim localPdfPath As String = FileMgr.GetLocalViewPath(sourcePdfRel)
                    Dim localRenamedPdfPath As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), targetFileName & ".pdf")

                    If System.IO.File.Exists(localPdfPath) Then
                        System.IO.File.Copy(localPdfPath, localRenamedPdfPath, True)
                        FileMgr.Save(lotDirPath, localRenamedPdfPath, True)

                        System.IO.File.Delete(localRenamedPdfPath)
                        System.IO.File.Delete(localPdfPath)
                    End If
                End If
            Else
                ' Download the XLS, generate PDF locally, upload, and cleanup
                If FileMgr.Exists(sourceXlsRel) Then
                    Dim localXlsPath As String = FileMgr.GetLocalViewPath(sourceXlsRel)
                    Dim localTargetPdf As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), targetFileName & ".pdf")

                    If System.IO.File.Exists(localXlsPath) Then
                        Dim ExcelWorkBook As Spire.Xls.Workbook = New Spire.Xls.Workbook()
                        ExcelWorkBook.LoadFromFile(localXlsPath)
                        ExcelWorkBook.Worksheets(0).PageSetup.IsFitToPage = True
                        ExcelWorkBook.Worksheets(0).SaveToPdf(localTargetPdf)

                        FileMgr.Save(lotDirPath, localTargetPdf, True)

                        System.IO.File.Delete(localTargetPdf)
                        System.IO.File.Delete(localXlsPath)
                    End If
                Else
                    ErrMsg = $"Source Excel file not found on server: {sourceXlsRel}"
                    Return Nothing
                End If
            End If

            ProcessStepReturn.ViewDocAction.PdfPath_DocumentCheck = targetPdfRel
    ...
    Public Function ProcessStepNo180_02_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim targetPdfRel As String = ""

            If CustOrd.MS_CODE Like "YTA[67]10-F*" Then
                Dim prodNo As String = If(CustOrd.PROD_NO, String.Empty)
                Dim lotNo As String = If(CustOrd.SHIP_LOT, String.Empty)

                ' Ensure Device ID directories exist
                Dim relDevIdBase As String = "Production Complete Documents\Device ID\"
                Dim prodDirPath As String = relDevIdBase & prodNo
                If Not DirMgr.Exists(prodDirPath) Then DirMgr.CreateDirectory(prodDirPath)

                Dim lotDirPath As String = prodDirPath & "\" & lotNo
                If Not DirMgr.Exists(lotDirPath) Then DirMgr.CreateDirectory(lotDirPath)

                Dim targetFileName As String = CustOrd.INDEX_NO & "-DeviceID-Lot" & lotNo
                targetPdfRel = lotDirPath & "\" & targetFileName & ".pdf"

                Dim relYtaQicBase As String = "Production Complete Documents\YTA_QIC\"
                Dim sourceDevPdfRel As String = relYtaQicBase & CustOrd.SERIAL_NO & "_DEV.pdf"
                Dim sourceDevXlsRel As String = relYtaQicBase & CustOrd.SERIAL_NO & "_DEV.xls"

                If FileMgr.Exists(sourceDevPdfRel) Then
                    If Not FileMgr.Exists(targetPdfRel) Then
                        ' File already in YTA_QIC, copy to target lot folder
                        Dim localPdfPath As String = FileMgr.GetLocalViewPath(sourceDevPdfRel)
                        Dim localRenamedPdfPath As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), targetFileName & ".pdf")
                    ...
                    End If
                Else
                    If FileMgr.Exists(sourceDevXlsRel) Then
                        ' Download the _DEV.xls, convert to pdf, save to both places
                        Dim localXlsPath As String = FileMgr.GetLocalViewPath(sourceDevXlsRel)
                        Dim localTargetPdf As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), targetFileName & ".pdf")
                        ...
                        If System.IO.File.Exists(localXlsPath) Then
                            ...
                            ' Upload to the target LOT folder
                            FileMgr.Save(lotDirPath, localTargetPdf, True)

                            ' Copy local PDF to a temp file matching the _DEV.pdf naming requirement and upload it
                            Dim localDevPdf As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), CustOrd.SERIAL_NO & "_DEV.pdf")
                            System.IO.File.Copy(localTargetPdf, localDevPdf, True)
                            FileMgr.Save(relYtaQicBase, localDevPdf, True)

                            ' Clean up
                            System.IO.File.Delete(localTargetPdf)
                            System.IO.File.Delete(localDevPdf)
                            System.IO.File.Delete(localXlsPath)
                        ...
                        End If
                    Else
                        ErrMsg = $"Source Device ID Excel file not found on server: {sourceDevXlsRel}"
                        Return Nothing
                    End If
                End If
            Else
                ' Default template if no device ID is needed
                targetPdfRel = "Production Complete Documents\Device ID\Template\No_DeviceID.pdf"
                ProcessStepReturn.Result = ""
            End If
    ...
    Public Function ProcessStepNo180_03_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim relLabelFolder As String = "Production Complete Documents\YTA_Label"
            Dim relBlankDoc As String = relLabelFolder & "\Template\YMA_YtaLabel.pdf"

            Dim finalFileName As String = CustOrd.INDEX_NO & ".pdf"
            Dim relFinalDoc As String = relLabelFolder & "\" & finalFileName

            ' Ensure Directory exists
            If Not DirMgr.Exists(relLabelFolder) Then
                DirMgr.CreateDirectory(relLabelFolder)
            End If
            ...
            ' Modify label locally, then upload
            If FileMgr.Exists(relBlankDoc) Then
                Dim localBlankDoc As String = FileMgr.GetLocalViewPath(relBlankDoc)
                Dim localFinalDoc As String = System.IO.Path.Combine(System.IO.Path.GetTempPath(), finalFileName)

                If System.IO.File.Exists(localBlankDoc) Then
                    Dim P_Doc = OpenPdfOperation_x64.FileOp.GetDocument(localBlankDoc, ErrMsg)
                    ...
                    ' Save modified PDF to local temp
                    P_Doc.Save(localFinalDoc)

                    ' Upload to server
                    FileMgr.Save(relLabelFolder, localFinalDoc, True)

                    ' Cleanup local temp files
                    System.IO.File.Delete(localFinalDoc)
                    System.IO.File.Delete(localBlankDoc)
                End If
            End If

            ProcessStepReturn.ViewDocAction.PdfPath_DocumentCheck = relFinalDoc
            ProcessStepReturn.Result = ""
            Return ProcessStepReturn
...
FixVar3:
    ...
    Public Function ProcessStepNo200_01_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            ProcessStepReturn.ProcedureStepAction.ProcedureMessage = "Is the YTA unit properly packed and kept inside Box?  YES  ||  NO"

            If Link.PlantID = "5Q00" Then
                ProcessStepReturn.ProcedureStepAction.ImagePath_ProcedureStep = MainForm.Setting.Var_51_ProStepView_ImagePath & "YTA\210\" & "Unit_InsidePB_5Q00.jpg"
            ElseIf Link.PlantID = "6Z00" Then
                ProcessStepReturn.ProcedureStepAction.ImagePath_ProcedureStep = MainForm.Setting.Var_51_ProStepView_ImagePath & "YTA\210\" & "Unit_InsidePB_6Z00.jpg"
            Else
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: Unknown Plant ID {Link.PlantID}"
                WMsg.ShowDialog()
                Return Nothing
            End If
    ...
    Public Function ProcessStepNo200_02_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim ResultValue As String = "[GO] ✓"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            AddWriteField("200_02_01", "IM", WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
                WMsg.ShowDialog()
    ...
    Public Function ProcessStepNo200_03_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            'Dim ResultValue As String = "[Yes] ✓"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            AddWriteField("200_03_01", "EUDOC", WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
                WMsg.ShowDialog()
    ...
    Public Function ProcessStepNo200_04_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim WriteFields As New List(Of OpenPdfOperation_x64.WriteField)
            Dim ResultValue As String = "[Seperate] ✓"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
    ...
    Public Function ProcessStepNo200_06_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim WriteFields As New List(Of OpenPdfOperation_x64.WriteField)
            Dim ResultValue As String = "[Ready] ✓"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
    ...
    Public Function ProcessStepNo200_08_00(ByVal Initial As String, ByVal CustOrd As POCO_YGSP.cust_ord, Optional ByRef ErrMsg As String = "") As CheckSheetStep
        Try
            ...
            Dim ResultValue As String = "[Yes] ✓"
            AddWriteField(ProcessStepReturn.StepNo, ResultValue, WriteFields:=WriteFields)
            AddWriteField("200_08_00_01", MainForm.Initial, WriteFields:=WriteFields)
            Dim ResultJson = AddResultTexts(WriteFields, ErrMsg:=ErrMsg)
            If ErrMsg.Length > 0 Then
                WMsg.Message = $"AddResultTexts() Error for Step:{ProcessStepReturn.StepNo}: {ErrMsg}"
                WMsg.ShowDialog()
