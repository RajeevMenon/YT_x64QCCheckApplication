Public Class CompleteInspection
    ...
    Dim TmlEntity As New MFG_ENTITY.Op(MainForm.Setting.Var_04_MySql_QA)
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        For Each process In ProcessNos
            ...
            Dim GroupStep = MainForm.AllCheckResult.Where(Function(x) x.ProcessNo = process).Select(Function(y) y.StepNo).FirstOrDefault.Split(",")
            Dim AllStepPass As Boolean = False
            Dim ProcessSave As New CheckSheetStep
            Dim StepCount As Integer = 0
            For Each Steps In GroupStep
                For Each item In MainForm.AllCheckResult
                    If item.ProcessNo = process And item.StepNo_Group Like "*" & Steps & "*" Then
                        StepCount += 1
                        ProcessSave = item
...
UpdateResult:
            If AllStepPass And StepCount = GroupStep.Length Then
                ...
                Tbl.CHECK_RESULT = ProcessSave.Result
            ...
            End If
            Dim SavedLine = TmlEntity.GetDatabaseTableAs_Object(Of POCO_QA.yta_qcc_v1p2)("INDEX_NO", MainForm.CustOrd.INDEX_NO, "PROCESS_NO", ProcessSave.ProcessNo, ErrMsg)
            If ErrMsg.Length > 0 Then
                MsgBox(ErrMsg, MsgBoxStyle.Critical, "Error")
                Exit Sub
            End If
            If Not IsNothing(SavedLine.INDEX_NO) Then
                Sql(Count) = TmlEntity.SetDatabaseModel_Sql(Tbl, "INDEX_NO", MainForm.CustOrd.INDEX_NO, "PROCESS_NO", ProcessSave.ProcessNo, ErrMsg)
                Count += 1
            Else
                Sql(Count) = TmlEntity.SaveDatabaseModel_Sql(Tbl, ErrMsg)
                Count += 1
            End If
        Next
        ReDim Preserve Sql(Count - 1)
        TmlEntity.ExecuteTransactionQuery(Sql, ErrMsg)
        If ErrMsg.Length > 0 Then
            MsgBox(ErrMsg, MsgBoxStyle.Critical, "Error")
        End If
    End Sub
    ...
    Private Sub Button_ViewQCC_Click(sender As Object, e As EventArgs) Handles Button_ViewQCC.Click
        ...
        '    Dim 
        '    Dim P_Doc = OpenPdfOperation.FileOp.GetDocument(Fn, ErMsg)
        '    OpenPdfOperation.FileOp.PDF_WriteText(Fn, Sf, "Hello", 10, 0.1, 0.1, OpenPdfOperation.FileOp.FontName.Wingdings, OpenPdfOperation.FileOp.FontColor.Black, OpenPdfOperation.FileOp.FontStyle.Regular, ErMsg)
